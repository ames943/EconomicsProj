"""Build meeting_prep_final_private.docx -- personal-use-only final pass on v3.

Not shown to anyone (per the task: "This doc is for ME ONLY"). Charts are
now presented separately from presentation_charts/ (see
build_presentation_charts.py), so this docx keeps its embedded charts only
as Amey's own reference while prepping, not as a presentation surface.

Changes from v3 (2026-08-27):
1. A one-page cheat sheet ("IF NOTHING ELSE — GET THESE TWO ANSWERS") is
   now the literal first page of the document, before Background.
2. Page numbers (bottom center) on every page except that first page,
   via section.different_first_page_header_footer + a PAGE field in the
   regular footer.
3. Inline "note to self" asides (self-corrections, meta-caveats) are now
   visually distinct from "SAY THIS" content -- italic + muted gray, no
   border, so they read as "don't say this out loud" at a glance. Applied
   consistently to all three instances of this pattern in the doc: the
   Philadelphia/Pennsylvania note-taking-slip parenthetical, the "also
   worth flagging, not a decision" caveat, and the anchor-date single-day
   caveat.
...
def main():
    ...
    out_path = OUTPUT_DIR / "meeting_prep_final_private.docx"
