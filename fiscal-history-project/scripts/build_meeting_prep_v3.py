"""Build meeting_prep_v3.docx -- visual/readability polish pass on v2.

Same content and numbers as meeting_prep_v2.docx (pulled from
PROJECT_CONTEXT.md, unchanged). Changes from v2 (2026-08-27):

1. TOC removed -- doc is short enough to flip through.
2. Background section rewritten as grouped bullets instead of narrative
   paragraphs, for a 2-minute skim before the meeting.
3. Chart footnotes: the dense technical note under each chart turned out to
   be baked into the PNG itself (matplotlib figure text), not docx caption
   text -- cropped it off each image (see scripts/crop_chart_footnotes.py
   logic inlined below) rather than editing the protected chart-generation
   scripts. Croppped copies live in output/charts_v3_cropped/; the
   originals in output/ are untouched.
4. One new "SAY THIS" opener before the Agenda section, acknowledging the
   month gap; the Agenda's own SAY THIS was trimmed so it doesn't repeat
   the same line.
5. One consistent accent color (deep blue, #1B4B66) drives headings, SAY
   THIS boxes, and the agenda box. The two-decision box keeps a separate
   amber "needs a decision" tone on purpose (a second, deliberate accent
   for call-to-action, not accidental inconsistency). The Background
   section gets its own distinct treatment (a left-border stripe panel in
   a neutral tan) specifically so it reads as "different, don't say this
   out loud" at a glance.

Boxes are still built from bordered paragraphs, not tables -- a table-based
box silently clipped its last line under Pages' renderer in an earlier
pass (confirmed via raw XML that the content existed but wasn't rendered).
"""
...
ACCENT = "1B4B66"          # single accent: headings, SAY THIS, agenda box, dividers
ACCENT_RGB = RGBColor(0x1B, 0x4B, 0x66)
ACCENT_TINT = "E9F1F5"     # light tint of accent, for box bodies

DECISION_BORDER = "C97A26"  # deliberate second "needs a decision" tone
DECISION_FILL = "FFF3E0"
DECISION_TEXT = RGBColor(0x8A, 0x51, 0x14)

BG_STRIPE = "9C8B6E"       # Background section: distinct neutral tan stripe
BG_FILL = "F2EFE9"
...
def say_this(doc, text):
    """Consistent spoken-script tag: dark accent label row + light tint body."""
    start = start_box(doc)
...
def main():
    ...
    out_path = OUTPUT_DIR / "meeting_prep_v3.docx"
